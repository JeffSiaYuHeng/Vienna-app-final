  ...
  const handleUpdateProfile = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        navigation.navigate("TabNavigator"); // Navigate to the appropriate screen
      })
      .catch((error) => {
        if (error.response) {
          ...
          <View className="w-full gap-y-3">
            <View className="w-full">
              ...
              <TextInput
                ...
                className="w-full border-[1px] h-10 border-gray-200 rounded-[5px] pl-2 bg-gray-200 text-gray-400"
            ...
            <View className="w-full">
              ...
              <TextInput
                value={userProfile.email} // Display the initial value from the state (userProfile.email)
                className="w-full border-[1px] h-10 border-gray-200 rounded-[5px] pl-2 bg-gray-200 text-gray-400"
                placeholder="Email"
                editable={false}
                onChangeText={(text) => setEmail(text)} // Update the email state using setEmail
              />
            </View>
            <View className="w-full">
